...
void Init()			// To initialize the knight data and draw a Yut's map.
{
	...
	map[10].push_back(25);
	for (int j = 1; j <= 2; j++)				// From center point to End point.
		map[10].push_back(25 + j * 5);
}

bool Moved(int idx, int num)				// To move the knight in Yut map.
{
...
void DFS(int cnt)			// To do exhaustive search.
{
	...
	for (int i = 1; i < 5; i++)
	{
		...
		if (!Moved(i, input[cnt]))					// Moved the current knight with dice value.
		{													// [Moved 함수 내부에서 (y,x) 좌표 이동이 이루어지기 때문.]
			visit[py][px] = true;						// Take back to previous state.	
			continue;
		}
		DFS(cnt + 1);
...
int main(void)
{
	...
	DFS(0);								// [Permutation with repetition. (중복 순열)]
	cout << answer << endl;		
}