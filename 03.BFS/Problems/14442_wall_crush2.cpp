...
{
	int y, x;								// Coordinates.
	int crush;							// Wall crush.
};

int N, M, K;							// N = The height of mpa.			M = The width of map.				K = The number of wall crush.
char map[MAX][MAX];			// map = The memory of input data.
int dist[11][MAX][MAX];			// dist = The memory of how far the distance is.
...
int BFS()			// To figure out the shortest way to arrived at destination.	
{
	...
	while (!que.empty())
	{
		...
		for (int i = 0; i < 4; i++)
		{
			...
			else if (map[ny][nx] == '1' && cur.crush < K)		// Wall. + Possible crush.		
			{
				que.push({ ny,nx, cur.crush + 1 });
				dist[cur.crush + 1][ny][nx] = dist[cur.crush][cur.y][cur.x] + 1;
			}
		}
	}
	return -1;
